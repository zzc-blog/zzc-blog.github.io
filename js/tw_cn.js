document.addEventListener('DOMContentLoaded', () => {
  ...
  let translateButtonObject = document.getElementById('translateLink')
  ...
  const getMaps = () => {
    ...
    return { s2tMap, t2sMap }
  }

  const Traditionalized = cc => {
    const { s2tMap } = getMaps()
    let str = ''
    for (let i = 0; i < cc.length; i++) {
      const ch = cc.charAt(i)
      str += cc.charCodeAt(i) > 10000 && s2tMap.has(ch) ? s2tMap.get(ch) : ch
    }
    return str
  }

  const Simplized = cc => {
    const { t2sMap } = getMaps()
    let str = ''
    for (let i = 0; i < cc.length; i++) {
      const ch = cc.charAt(i)
      str += cc.charCodeAt(i) > 10000 && t2sMap.has(ch) ? t2sMap.get(ch) : ch
    }
    return str
  }
